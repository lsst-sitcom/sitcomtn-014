...
The Product View allows one to identify specific documents as the normative source of information, then provide a manner to associate, link and cross reference documentation and informational dependencies from respective normative sources or references thereafter.
These goals support identifying and resolving gaps, miscommunications and error-likely situations.
...
The Product View does not need to follow the physical decomposition of the system; it should be based around functionality, user needs, or other attributes as determined by the technical team or responsible group.
...
In light of this need, the Documentation Working Group recommends all Product View trees be available via one of the project repositories.
It is suggested to create an appropriate lsst.io website so departments and groups can easily make changes within a currently supported documentation tool (see Section \ref{sec:LTD}).

Products are defined by the owning department in terms of a system or set of systems which can be grouped or decomposed into subsystems or individual components to a degree appropriate for them and their stakeholders.
Referred to as the \emph{generic categories}, the customized categories for the Product View developed by the Documentation Working Group are predefined root-nodes and the first set of leaf-nodes for any product tree.
The generic categories were developed to capture critical operational aspects of a product while being extendable to subsystems and components of a product, where the subsystem may be a product with subsystems in its own right; i.e., a \emph{Level-1 product} can be comprised of multiple Level-2 or lower-level products (subsystems).
Those subsystems which are products are \emph{Level-2+ products}.
...
It is at the department's discretion on how to best organize and characterize these products and their product trees to manage their systems and flow of information; however, it is expected that all generic categories are associated with Level-1 and Level-2+ products, either by construction, referenced information or reason for non-applicability.
While there will be exceptions for low-level products, the Product View and its generic categories are designed to robustly capture construction completeness and operations readiness because they should generate sufficient discussion between groups and stakeholders to ensure key information is identified and provided.

\subsubsection{Departments for Rubin Observatory operations}

The Rubin Observatory departments shall be described by their associated scope, systems and/or products descriptions, and contacts to identify managers, product owners and other key personnel (e.g., via organizational chart or contact list).
This section lists each department and respective scope.
Contact information should be available to individuals that require it, and it should be clear when it is appropriate to contact an individual or group (e.g., as a product owner).
Note that the letter ``R'' can prepend the department acronym, e.g., ROO for Rubin Observatory Operations.
...
Further opportunities arise with a well designed set of product trees: clearly establish relationships and dependencies between systems, serve as an introduction to department/system in a digestible manner, and create a more adaptable structure to organize and target information between technical groups, either internal or external to the department.

The generic categories are separated by five high-level categories that act as root-nodes --- (1) Technical Design, (2) Procedures, (3) Safety and Emergency Response, (4) Evaluation and (5) Archival Documents.
...
It may be difficult at first for technical groups to perform a logical and relevant decomposition of the systems such that the generic categories are applicable to all product trees, especially if a system can change in different scenarios, contexts or states (e.g., the Simonyi Survey Telescope could have one product tree for maintenance and another product tree for on-sky operations where each captures different components and inter-departmental interfaces separately).
...
Technical teams can and should create product trees by referring to documentation.
As context to the product tree, they should ensure to describe how these external documents address the generic categories.
...
Within the department's Product View which consists of many product trees, the Documentation Working Group intended for systems and subsystems to refer to higher- or lower-level product trees to reduce replication and the risk of confusing users accessing information between the groups and stakeholders.
Furthermore, the referential nature can be applied for inter-departmental information and dependencies so it's clear which department owns the information and any departments which utilize it.
...
The following are the generic categories:
...
Figure \ref{fig:product-view-auxtel-technical-design-example} is a more comprehensive example of AuxTel, depicting the \emph{Technical Design} generic category.
Each category (yellow) and each subcategory (blue) from this generic category is addressed with the intent of helping the reader understand the basic elements of the Level-1 product, how it is used, and documentation with references therein for retrieving critical information.
...
With well-defined responsibilities, technical teams can communicate more effectively and the normative source of information should be identifiable.

\begin{table}[]
\begin{tabular}{|l|l|l|l|}
\hline
 & \textbf{\begin{tabular}[c]{@{}l@{}}Observatory\\ Operations\end{tabular}} & \textbf{\begin{tabular}[c]{@{}l@{}}System\\ Performance\end{tabular}} & \textbf{\begin{tabular}[c]{@{}l@{}}Data\\ Management\end{tabular}} \\ \hline
