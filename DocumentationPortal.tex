...
The new Rubin Documentation Portal discussed herein is based upon the website \url{www.lsst.io} which provides a search and browsing interface for the Rubin Observatory's public-facing technical documentation operated by the SQuaRE (Data Management) team. \citep{lsst.io-cite}
(While summarized in this document, a detailed description of \url{www.lsst.io}, LSST the Docs (LTD) and applicable software is available in \citedsp{SITCOMTN-012}.)
The new Rubin Documentation Portal will be accessible only those with Rubin Observatory staff credentials, and will be purpose-built for observatory and survey operations.
...
The architecture described here is based upon that which is already put into production with the \url{www.lsst.io} portal for public-facing documentation.
...
It will also use a separate instance of the Algolia database to eliminate any risks associated with leaking internal documentation to the public-facing \url{www.lsst.io} portal.
...
For the \url{www.lsst.io} public documentation portal, the developers opted to use Algolia and recommend the same choice for the new Rubin Documentation Portal.

Although \url{www.lsst.io} is currently operating on a free open-source license of Algolia, the new internal Rubin Documentation Portal would be an entirely paid license.
Algolia prices based on record counts and request rates.
In operating \url{www.lsst.io}, we found record count to be the limiting factor.
At the moment, $1,000$ records costs \$1 per month.
$1,000,000$ records would cost \$850 per month with volume discounts.
For reference, the \url{www.lsst.io} service currently uses $110,000$ records to host all technical notes and change-controlled documents with drafts hosted on LSST the Docs (LTD), also known as lsst.io.
...
Whereas we chose Algolia to provide a turn-key search database service, for \url{www.lsst.io} we chose to build the indexing service in-house to have complete control over how documents are indexed, and what metadata is associated with each document.
...
For \url{www.lsst.io}, we built the site as a React JavaScript application.
