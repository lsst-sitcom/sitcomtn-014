...
The Storage View provides a method for departments and technical groups to define locations for respective \emph{normative sources of information} --- the primary location where a information is documented, and this location shall be used to reference and source that information ensuring it is consistently and accurately retrievable.
With canonical information storage, others can develop associations, links and references to impart information to inter-departmental or external entities.
These goals support a reduction in replication, an increase in reliable information flow, and preservation of informational dependencies.
Unless mandated by the project, the department or responsible group, in conjunction with stakeholders, can choose the manner and method of storing information and transposing information to other locations.
...
System administrators must ensure access throughout operations (e.g., readily available, secure, backed-up) and a level of expertise must be on-hand for maintenance, issues and general assistance to staff.
Departments and technical staff must maintain their information and its organization within the repositories.
...
Additionally compounding the risk to information access, it becomes increasingly difficult to share information, especially when a repository is ill-defined, uncertainty of what replicated information is the most current, or uncertainty of which repository is the normative source of information.
...
Critically, each Storage View tree should readily indicate what are the normative sources of information so it is accurately referable within other Documentation Views.
It will be the responsibility of the department or technical group to define the organizational structure.
Software developers require the metadata and repository's organizational structure to design and implement a system like the Rubin Documentation Portal (Section \ref{sec:DocPortal}), including the development of additional metadata fields with assistance from system administrators.

The subsections below summarize some important details from \citeds{SITCOMTN-012}; however, that document includes detailed information, such as the inventory for each repository.
...
The Documentation Working Group did not review databases of information and data, such as the Engineering and Facility Database (EFD) or the Prompt Products Database (PPDB).
...
Additionally, DocuShare's functionality lends itself well for use as a normative source of information.
...
The suggested DocuShare branch of the Storage View for operations starts with the collection for each department.
Additional collections can be created based on the project's needs as a whole (e.g., inter-departmental items such as widely used software tools).
...
The archiving process should include a determination by the department if the information is useful in operations (determining if it should be archived) and if the information should be quarriable in the Archive Server (e.g., raw data).
...
LTD hosts \emph{versioned static websites}, meaning any website built from HTML, CSS and JavaScript that does not need an active server to render content (as opposed to Confluence, DocuShare, or Drupal websites, for example).
...
\begin{itemize}
	...
	We do not operate any servers that receive traffic from users;
...
With a wiki-like Confluence that integrates well with Jira, the two are especially convenient for rapidly developing new ideas, taking and storing meeting minutes, collecting information in interactive tables, and drafting outlines for future documentation, all while tracking the tasks and being able to actively report on status.
...
Users must understand the limitations of information and provided guidance by the department as to how and when information should be moved from Confluence or Jira to another storage location.
...
Solidworks Product Data Management (PDM) Professional \citep{PDM-cite} is the official computer-aided design (CAD) model repository for the Telescope \& Site construction group, including vendor documentation and deliverables.
...
The LSSTCam engineering model will be synchronized with PDM vault during the transition to operations.
...
\begin{itemize}
	...
	\item \textbf{Operations Concepts} --- normative source.
	\item \textbf{System-level State Machine} --- normative source.
	\item \textbf{Interlocks} --- modeled from normative source material.
...
The drive contains a number of personal directories and a directory named ``TS-Deliverables'' managed by the Telescope \& Site group.
...
If web content management is controlled to the same degree as in construction, it is recommended that the project provide a clear workflow of how and where normative sources of information are used and stored.
...
Its use will not continue into Operations.
It is recommended that the P6 record of the Construction project be exported and saved at the end of the Project.

\subsubsection{Verification reports}

The Rubin Observatory Verification Architecture used for construction and commissioning activities is composed of the MagicDraw model, Jira with Test Manager application, and Syndeia to synchronize data between MagicDraw and Jira.
For all verification events performed by the SIT-COM and DM teams, a SIT-COM Test Plan/Report (SCTR) or DM Test Plan/Report (DMTR) is created.

The SCTR or DMTR is generated using the Jira REST API pulling the information from the verification objects.
...
It is expected that not all procedures will be stored in CMMS, e.g., a hyperlink to a procedure in DocuShare may be used instead of writing the procedure into openMAINT.
As the work release workflow will be in Jira, the CMMS will have an interface (e.g., REST API) and update Jira as needed.
With a well-defined workflow, the CMMS and Jira interactions can be streamlined to minimize confusion of scope and use between the two tools.
...
The Rubin Observatory information is exported and archived in GitHub by operations staff, and a user guide for the Rubin Observatory is available at \url{https://rtn-051.lsst.io/}.
